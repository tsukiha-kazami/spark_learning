...
case class Job_Detail(job_id: String, job_name: String, job_url: String, job_location: String,
                      job_salary: String,
...
object JdbcOperate {
  ...
  val getConn: () => Connection = () => {
    DriverManager
      .getConnection("jdbc:mysql://localhost:3306/mydb?characterEncoding=UTF-8", "root", "root")
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val jdbcRDD: JdbcRDD[Job_Detail] = new JdbcRDD(
      ...
      rs => {
        ...
        Job_Detail(job_id, job_name, job_url, job_location, job_salary, job_company, job_experience,
          job_class, job_given, job_detail, company_type, company_person, search_key, city)
      }
    )
    ...
    //第二种实现方式
    //求取每个搜索关键字出现的岗位人数
    val resultRdd: RDD[(String, Int)] = jdbcRDD.map(x =>(x.search_key, 1)).
      reduceByKey(_ + _).filter(x => x._1 != null
    )
    //数据量变少，缩减分区个数
    ...
    rsultRdd2.foreachPartition(iter => { //创建数据库连接
      ...
      val statement = conn
        .prepareStatement("insert into job_count(search_name, job_num) values (?, ?)")
      //遍历
      iter.foreach(record => {
        //赋值操作
    ...
    val maxJobDetail: RDD[Job_Detail] = getEachJobs.map(x => {
      ...
      array.maxBy(x => {
        ...
        val result =
          if (StringUtils.isNotEmpty(job_salary) && job_salary.contains("k") && job_salary.contains("-")
            && job_salary.replace("k", "").split("-").length >= 2) {
