...
object CaseClassSchema {
  def main(args: Array[String]): Unit = {
    ...
    val sc: SparkContext = sparkSession1.sparkContext
    sc.setLogLevel("warn")
    ...
    sparkSession1.sql("select * from person").show
    sparkSession1.sql("select name from person").show
    sparkSession1.sql("select name,age from person").show
    sparkSession1.sql("select * from person where age >30").show
    sparkSession1.sql("select count(*) from person where age >30").show
    sparkSession1.sql("select age,count(*) from person group by age").show
    sparkSession1.sql("select age,count(*) as count from person group by age").show
    sparkSession1.sql("select * from person order by age desc").show
    //todo：----------------- SQL风格语法----------------------end
